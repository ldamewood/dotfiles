...
Shared across every host that imports `linux-builder.nix` (currently `mbp`
and `squash`) — the linux-builder is a NixOS VM that runs on macOS to build
Linux packages (aarch64-linux and x86_64-linux via emulation).
...
- If the port stays stuck, use another host port: in `linux-builder.nix` set `virtualisation.darwin-builder.hostPort = 31023;` and update the ssh `Port` in that same file's `Host linux-builder` stanza, then rebuild and restart the VM.
...
The linux-builder is accessible via SSH at `linux-builder` (configured in `linux-builder.nix`, optionally with a host-specific `hostSettings.linuxBuilderIdentityAgent`). SSH to the builder requires sudo (e.g. run commands with `sudo` on the builder, or remotely: `ssh linux-builder "sudo ..."`).
